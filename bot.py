...
# 🔄 СИСТЕМА АВТОМАТИЧНОГО ПЕРЕКЛЮЧЕННЯ РЕЖИМІВ
current_trading_mode = CURRENT_TRADING_MODE  # Поточний режим торгівлі
mode_switch_lock = threading.Lock()  # Лок для зміни режиму
last_mode_check_time = 0  # Час останньої перевірки умов для переключення режиму
MODE_CHECK_INTERVAL = 300  # Перевіряємо умови кожні 5 хвилин (300 секунд)
...
# 🔄 ФУНКЦІЇ АВТОМАТИЧНОГО ПЕРЕКЛЮЧЕННЯ РЕЖИМІВ ТОРГІВЛІ
# ------------------------------------------------------
def get_current_mode_params():
    """Повертає параметри поточного режиму торгівлі"""
    global current_trading_mode
    with mode_switch_lock:
        return MODE_PARAMS[current_trading_mode]

def get_current_trading_mode():
    """Повертає поточний режим торгівлі"""
    global current_trading_mode
    with mode_switch_lock:
        return current_trading_mode

def switch_trading_mode(new_mode: TradingMode, reason: str = ""):
    """
    Переключає режим торгівлі
    """
    global current_trading_mode, LEVERAGE, MIN_SPREAD, POSITION_MAX_AGE_SEC
    
    try:
        with mode_switch_lock:
            if current_trading_mode == new_mode:
                logging.info(f"ℹ️ Режим вже встановлено на {new_mode.value.upper()}")
                return False
            
            old_mode = current_trading_mode
            current_trading_mode = new_mode
            
            # Оновлюємо глобальні параметри з нового режиму
            mode_params = MODE_PARAMS[new_mode]
            LEVERAGE = mode_params["leverage"]
            MIN_SPREAD = mode_params["min_spread"]
            POSITION_MAX_AGE_SEC = mode_params["max_hold"] * 60  # Конвертуємо в секунди
            
            # Логування
            logging.warning(f"🔄 РЕЖИМ ЗМІНЕНО: {old_mode.value.upper()} → {new_mode.value.upper()}")
            logging.info(f"   📊 Нові параметри:")
            logging.info(f"      • Leverage: {mode_params['leverage']}x")
            logging.info(f"      • Min Spread: {mode_params['min_spread']}%")
            logging.info(f"      • Max Hold: {mode_params['max_hold']} хв")
            logging.info(f"      • Take Profit: {mode_params['tp']}%")
            logging.info(f"      • Stop Loss: {mode_params['sl']}%")
            if reason:
                logging.info(f"   💡 Причина: {reason}")
            
            # Відправляємо повідомлення в Telegram
            emoji = "🐂" if new_mode == TradingMode.BULL else "🛡️"
            message = f"{emoji} <b>РЕЖИМ ТОРГІВЛІ ЗМІНЕНО</b>\n\n"
            message += f"<b>Новий режим:</b> {new_mode.value.upper()}\n"
            message += f"<b>Попередній режим:</b> {old_mode.value.upper()}\n\n"
            message += f"<b>📊 Параметри:</b>\n"
            message += f"• Leverage: {mode_params['leverage']}x\n"
            message += f"• Min Spread: {mode_params['min_spread']}%\n"
            message += f"• Max Hold: {mode_params['max_hold']} хв\n"
            message += f"• TP: {mode_params['tp']}%\n"
            message += f"• SL: {mode_params['sl']}%\n"
            if reason:
                message += f"\n<b>💡 Причина:</b>\n{reason}"
            
            send_to_admins_and_group(message)
            
            return True
            
    except Exception as e:
        logging.error(f"❌ Помилка переключення режиму: {e}")
        return False
...
def log_trade_csv(symbol, side, price, size, leverage, pnl_usdt=0.0, pnl_pct=0.0, trade_type="OPEN"):
    """
    Записує угоду у файл trade_history.csv з деталізацією PnL
    """
    ...
    try:
        ...
        with open(filename, mode='a', newline='', encoding='utf-8') as file:
            ...
            # Оновлені заголовки з PnL
            if not file_exists:
                writer.writerow(["Time", "Symbol", "Side", "Type", "Price", "Size_Margin_USDT", "Leverage", "PnL_USDT", "PnL_Percent"])
            
            # Форматуємо час
            time_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            clean_symbol = symbol.replace('/USDT:USDT', '')
            
            # Якщо це відкриття, PnL = 0
            if trade_type == "OPEN":
                writer.writerow([
                    time_str, clean_symbol, side, trade_type, 
                    f"{price:.6f}", f"{size:.2f}", leverage, 
                    "0.00", "0.00%"
                ])
            else:
                # Для закриття пишемо PnL
                writer.writerow([
                    time_str, clean_symbol, side, trade_type, 
                    f"{price:.6f}", f"{size:.2f}", leverage, 
                    f"{pnl_usdt:.4f}", f"{pnl_pct:.2f}%"
                ])
                
            logging.info(f"📝 CSV: {trade_type} {clean_symbol} | PnL: ${pnl_usdt:.4f} ({pnl_pct:.2f}%)")
    except Exception as e:
...
def calculate_pnl_percentage(position, use_leverage=False):
    """
            ...
            for symbol, position in current_positions.items():
                ...
                if ENABLE_TIME_STOP:
                    ...
                    if expires_at > 0 and current_time >= expires_at:
                        ...
                        pnl_pct = calculate_pnl_percentage(position, use_leverage=False)
                ...
                # 🔄 ОТРИМУЄМО TP/SL З ЗБЕРЕЖЕНИХ ПАРАМЕТРІВ РЕЖИМУ ПОЗИЦІЇ
                position_tp = position.get('mode_tp_pct', TAKE_PROFIT_PCT)  # Використовуємо TP з режиму позиції
                ...
                # 1.1 ПЕРЕВІРКА СТОП-ЛОСС (Використовуємо SL з режиму позиції)
                if pnl_pct <= -position_sl:
                    logging.info(f"🚨 SL TRIGGERED [{symbol}] PnL={pnl_pct:.2f}% ≤ -{position_sl:.2f}% → CLOSE (Режим: {position_mode})")
                    reason = f"SL {pnl_pct:.1f}% (Режим: {position_mode})"
                    positions_to_close.append((symbol, position, reason, pnl_pct))
                    continue
            ...
            for symbol, position, reason, pnl_pct in positions_to_close:
                ...
                if result:
                    ...
                    if position_closed:
                        ...
                        # 💰 РОЗРАХУНОК РЕАЛІЗОВАНОГО PNL (Брутто)
                        realized_pnl_usdt = (position['size_usdt'] * pnl_pct / 100)
                        
                        # Логуємо в CSV (Використовуємо current_price замість помилкового current_xt_price)
                        log_trade_csv(
                            symbol=symbol,
                            side=position['side'],
                            price=current_price,  # ✅ ВИПРАВЛЕНО ТУТ
                            size=position['size_usdt'],
                            leverage=LEVERAGE,
                            pnl_usdt=realized_pnl_usdt,
                            pnl_pct=pnl_pct,
                            trade_type="CLOSE"
                        )

                    # Визначаємо емодзі та статус
                    ...
                    # ✅ ВІДПРАВЛЯЄМО ДЕТАЛЬНЕ ПОВІДОМЛЕННЯ
                    # (Використовуємо current_price замість помилкового current_xt_price)
                    close_signal = f"🏁 **ПОЗИЦІЯ ЗАКРИТА** {result_emoji}\n"\
                                  f"📊 **{symbol.replace('/USDT:USDT', '')}** ({position['side']}) | ⚡ XT.COM\n"\
                                  f"💰 Маржа: **${position['size_usdt']:.2f}** (Lev: {LEVERAGE}x)\n"\
                                  f"📈 Вхід: **${position['avg_entry']:.6f}**\n"\
                                  f"📉 Вихід: **${current_price:.6f}**\n"\
                                  f"💎 P&L: **{pnl_pct:+.2f}%** (${realized_pnl_usdt:+.4f})\n"\
                                  f"📝 Причина: **{reason}**\n"\
                                  f"ℹ️ *Примітка: Сума не включає комісії біржі (~0.06-0.1%)*\n"\
                                  f"⏰ Час: **{time.strftime('%H:%M:%S %d.%m.%Y')}**"
                                  
                    send_to_admins_and_group(close_signal)
                    
                    logging.info(f"✅ {symbol}: Позицію успішно закрито, P&L={pnl_pct:+.1f}% (${realized_pnl_usdt:.4f})")
                # if result:
                #     # Успішне закриття - видаляємо з активних позицій 🔒 THREAD SAFE
        ...
        try:
            ...
            if is_realistic and enhanced_entry_check and not already_has_position:
                ...
                if MIN_SPREAD <= abs(xt_dex_spread_pct) <= MAX_SPREAD:
                    ...
                    if current_mode_check == TradingMode.CONSERVATIVE and side == "LONG":
                        logging.info(f"[{symbol}] 🛡️ CONSERVATIVE MODE: Пропускаем LONG сигнал (разрешены только SHORT)")
                        return  # Выходим из функции, сигнал и сделка не создаются
            ...
            if spread_check and balance_check and not DRY_RUN and is_realistic:
                ...
                current_mode_check = get_current_trading_mode()
                if current_mode_check == TradingMode.CONSERVATIVE and side == "LONG" and not has_position:
                     # not has_position добавлен, чтобы не блокировать усреднения, если позиция уже есть (опционально)
                     logging.info(f"[{symbol}] 🛡️ CONSERVATIVE MODE: Блокировка открытия LONG позиции")
                     return
                ...
                if not has_position and positions_check:
                    ...
                    if ok_liq and trading_exchange == "xt":
                        # 🔄 ОТРИМУЄМО ПАРАМЕТРИ З ПОТОЧНОГО РЕЖИМУ
                        mode_params = get_current_mode_params()
                        current_leverage = mode_params["leverage"]
                        
                        # 🔥 ВИПРАВЛЕННЯ: Визначаємо сторону заздалегідь для Hedge Mode
                        position_side = "LONG" if side == "LONG" else "SHORT"

                        # 🔥 ДОДАНО: Встановлення ІЗОЛЬОВАНОЇ МАРЖІ з урахуванням positionSide
                        try:
                            # Передаємо positionSide, бо XT вимагає це для Hedge Mode
                            xt.set_margin_mode('isolated', symbol, params={'positionSide': position_side})
                            logging.info(f"[{symbol}] 🛡️ XT: Режим маржі переключено на ISOLATED ({position_side})")
                        except Exception as e:
                            # Якщо помилка "No need to modify", це означає, що режим вже Isolated - це нормально
                            if "No need to modify" in str(e):
                                logging.info(f"[{symbol}] ✅ Режим вже ISOLATED")
                            else:
                                logging.warning(f"[{symbol}] ⚠️ Не вдалося встановити ISOLATED: {e}")
                    ...
                    if ok_liq:
                        ...
                        if trading_exchange == "xt":
                            ...
                            mode_params = get_current_mode_params()
                            current_leverage = mode_params["leverage"]
                            current_tp_pct = mode_params["tp"]
                            current_sl_pct = mode_params["sl"]
                            current_mode_name = get_current_trading_mode().value.upper()
                            
                            try:
                                # Правильний виклик з positionSide
                                position_side = "LONG" if side == "LONG" else "SHORT"
                                xt.set_leverage(current_leverage, symbol, {"positionSide": position_side})
                                logging.info(f"[{symbol}] ⚙️ XT: ПРИМУСОВО встановлено леверидж {current_leverage}x ({position_side}) - Режим: {current_mode_name}")
                            except Exception as e:
                    ...
                    #             position_side = "LONG" if side == "LONG" else "SHORT"
                    #             xt.set_leverage(current_leverage, symbol, {"positionSide": position_side})
                    #             logging.info(f"[{symbol}] ⚙️ XT: ПРИМУСОВО встановлено леверидж {current_leverage}x ({position_side})")
                    #         except Exception as e:
                    #             logging.error(f"[{symbol}] ❌ Помилка встановлення левериджу XT: {e}")
                    ...
                    #             pass
                            ...
                            with order_placement_lock:
                                ...
                                order_account_1 = xt_open_market_position(xt_account_1, symbol, side, ORDER_AMOUNT, current_leverage, ref_price, dex_price, spread_pct)
                                order_account_2 = xt_open_market_position(xt_account_2, symbol, side, ORDER_AMOUNT, current_leverage, ref_price, dex_price, spread_pct)
                                # Вважаємо успішним якщо хоча б один акаунт відкрив позицію
                                order = order_account_1 or order_account_2
                                if order_account_1:
                                    logging.info(f"[{symbol}] ✅ АКАУНТ 1: Відкрито {side} позицію з левериджем {current_leverage}x (Режим: {current_mode_name})")
                                if order_account_2:
                                    logging.info(f"[{symbol}] ✅ АКАУНТ 2: Відкрито {side} позицію з левериджем {current_leverage}x (Режим: {current_mode_name})")
                        else:
                            order = None
                        if order:
                                logging.info(f"[{symbol}] 🚀 XT: Відкрито {side} позиції на обох акаунтах з левериджем {current_leverage}x (Режим: {current_mode_name})")
                        ...
                        if order:
                            ...
                            # 🔄 ДИНАМІЧНІ TP/SL З ПОТОЧНОГО РЕЖИМУ
                            if side == "LONG":
                                tp_price = entry_price * (1 + current_tp_pct / 100)  # TP% з режиму
                            else:  # SHORT
                                tp_price = entry_price * (1 - current_tp_pct / 100)  # TP% з режиму
                            position = {
                                "side": side,
                            ...
                            if 'opened_at' not in existing_position or existing_position.get('opened_at', 0) <= 0:
                                ...
                                position['opened_at'] = existing_position['opened_at']  # Зберігаємо існуючий!
                            # 🔄 Використовуємо max_hold з режиму для expires_at
                            mode_max_hold_sec = mode_params["max_hold"] * 60  # хвилини в секунди
                            if 'expires_at' not in existing_position or existing_position.get('expires_at', 0) <= 0:
                                position['expires_at'] = position['opened_at'] + mode_max_hold_sec
                            else:
                                position['expires_at'] = existing_position['expires_at']  # Зберігаємо існуючий!
                            position['xt_pair_url'] = generate_xt_pair_url(symbol)
                            ...
                            try:
                                ...
                                opened_message = format_position_opened_message(
                                    ...
                                    trading_mode=current_mode_name,
                                    tp_pct=current_tp_pct,
                                    sl_pct=current_sl_pct
                                )
                                send_to_admins_and_group(opened_message)
                                logging.info(f"📱 Відправлено Telegram про відкриття {symbol} (Режим: {current_mode_name})")
                            except Exception as e:
...
def start_workers():
    ...
    while bot_running:
        try:
            ...
            # 🔄 ПЕРЕВІРКА УМОВ ДЛЯ ПЕРЕКЛЮЧЕННЯ РЕЖИМУ
            try:
                check_and_switch_mode()
            except Exception as mode_check_error:
                logging.error(f"❌ Помилка перевірки режиму торгівлі: {mode_check_error}")
