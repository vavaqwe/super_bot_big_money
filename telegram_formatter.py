...
🔹 STATUS: {status.upper()}"""
    ...
    def format_position_opened(self, symbol: str, side: str, entry_price: float, 
                             size_usd: float, leverage: int, spread_percent: float,
                             trading_mode: str = "CONSERVATIVE", tp_pct: float = 1.4, sl_pct: float = -1.7) -> str:
        """🚀 ФОРМАТ ВІДКРИТТЯ ПОЗИЦІЇ З РЕЖИМОМ ТОРГІВЛІ"""
        
        # Чистимо символ для відображення
        clean_symbol = symbol.replace('/USDT:USDT', '').replace('/USDT', '')
        opening_amount = size_usd * leverage  # $5 маржа × leverage = позиція
        
        # Розраховуємо TP і SL з переданих параметрів
        if side == "LONG":
        ...
        # Емодзі режиму
        mode_emoji = "🐂" if trading_mode.upper() == "BULL" else "🛡️"
        
        message = f"""💰 <b>ПОЗИЦІЯ ВІДКРИТА</b> 💰

📊 {clean_symbol} {side}
{mode_emoji} <b>Режим:</b> {trading_mode.upper()}
...
def format_position_opened_message(symbol: str, side: str, entry_price: float, 
    ...
    return telegram_formatter.format_position_opened(symbol, side, entry_price, size_usd, leverage, spread_percent, trading_mode, tp_pct, sl_pct)
